...
class TestServer:
    ...
    async def test_cleanup_calls_close_instance(self):
        ...
        with patch("dependency_track_mcp.server.DependencyTrackClient.close_instance", new=AsyncMock()) as close_mock:
            await cleanup()
    ...
    def test_main_runs_and_cleans_up(self):
        ...
        with patch.dict(
            ...
            with patch.object(mcp, "run") as run_mock, patch(
                "dependency_track_mcp.server.asyncio.run"
            ) as asyncio_run_mock, patch(
                "dependency_track_mcp.config.get_settings"
            ) as settings_mock:
    ...
    def test_module_main_exec(self):
        ...
        with patch.dict(
            ...
            with patch("fastmcp.server.server.FastMCP.run") as run_mock, patch(
                ...
                try:
                    runpy.run_module(
                        "dependency_track_mcp.server", run_name="__main__"
                    )
                except SystemExit:
