...
class TestMCPServerIntegration:
    ...
    async def test_end_to_end_project_workflow(self, mcp):
        ...
        with patch.object(DependencyTrackClient, "get_instance") as mock_get_instance:
            ...
            mock_client.put = AsyncMock(return_value={"uuid": "proj-1", "name": "Test Project"})
            mock_client.get = AsyncMock(
                return_value={"uuid": "proj-1", "name": "Test Project", "version": "1.0"}
            )
            mock_client.get_with_headers = AsyncMock(
                return_value=([{"uuid": "proj-1", "name": "Test Project"}], {"X-Total-Count": "1"})
            )
            mock_client.post = AsyncMock(return_value={"uuid": "proj-1", "name": "Updated"})
    ...
    async def test_error_handling_integration(self, mcp):
        ...
        with patch.object(DependencyTrackClient, "get_instance") as mock_get_instance:
            mock_client = AsyncMock()
            mock_client.get = AsyncMock(side_effect=NotFoundError("Project not found"))
            mock_get_instance.return_value = mock_client
    ...
    async def test_pagination_across_tools(self, mcp):
        ...
        with patch.object(DependencyTrackClient, "get_instance") as mock_get_instance:
            mock_client = AsyncMock()
            mock_client.get_with_headers = AsyncMock(
                return_value=([{"uuid": "1"}, {"uuid": "2"}], {"X-Total-Count": "100"})
            )
            mock_get_instance.return_value = mock_client
