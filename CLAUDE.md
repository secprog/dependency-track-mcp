...
python -m dependency_track_mcp.server  # Direct module execution
```
...
Tool Handlers (tools/*.py)
    ↓ call client methods
DependencyTrackClient (client.py) [singleton]
    ↓ async HTTP
...
- **server.py**: Entry point, initializes FastMCP, registers tools, enforces security requirements at startup
- **client.py**: Async HTTP client with singleton pattern, retry logic, rate limit handling
- **config.py**: Pydantic settings from environment variables (DEPENDENCY_TRACK_ prefix), validates security requirements
- **oauth.py**: JWT validation, scope-based authorization middleware
...
- **HTTPS-only** - HTTP rejected unless `DEV_ALLOW_HTTP=true`
...
All settings use `DEPENDENCY_TRACK_` prefix. Required:
- `DEPENDENCY_TRACK_URL` - Backend URL (must be HTTPS)
- `DEPENDENCY_TRACK_API_KEY` - API key for backend
- `DEPENDENCY_TRACK_OAUTH_ISSUER` - OAuth issuer URL
